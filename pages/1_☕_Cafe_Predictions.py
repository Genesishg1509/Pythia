...
Portfolio version:
- Uses uploaded invoice_layout.txt when provided (priority, runs the full ETL pipeline).
- Otherwise uses precomputed demo scenario inputs: data/demo/20260203_all_inputs_cxf_*.csv
...
MODELCONFIG = {
    ...
    "Breakfast/Toast units": {
        "file": "2026_02_01_prophet_units_breakfast_toast_bread_model.joblib",
        "unit": "units",
...
# Demo dataset: precomputed Prophet "future" feature rows per scenario
# (ds, invoices_lag_14, tmed, temp_range, is_hot, is_cold, visitantes_cxf)
DEMO_SCENARIO_CSVS = {
    "Baseline": Path("data/demo/20260203_all_inputs_cxf_baseline.csv"),
    "Low Pessimistic": Path("data/demo/20260203_all_inputs_cxf_low.csv"),
    "High Optimistic": Path("data/demo/20260203_all_inputs_cxf_high.csv"),
}
...
@st.cache_data(show_spinner=False)
def load_demo_scenarios(scenario_paths: dict):
    """
    Load precomputed scenario feature tables for the demo dataset.
    These are the same "future" input rows build_prophet_prediction_inputs()
    would produce, already computed and saved as CSV (no raw invoice data needed).
    """
    return {
        name: pd.read_csv(path, parse_dates=["ds"])
...
with st.sidebar:
    ...
    uploaded_txt = st.file_uploader(
        "Upload invoice_layout.txt (or press RUN PREDICT to use demo scenarios)",
        type=["txt"],
        help="If uploaded, this file is used. Otherwise, precomputed demo scenarios are used."
    )

    st.caption("Demo dataset: precomputed Jan 2026 forecast inputs (data/demo/)")
    if AVG_WEATHER_XLSX.exists():
        st.caption(f"Weather file (for custom uploads): {AVG_WEATHER_XLSX.as_posix()}")
...
if run_prediction:
    try:
        # Priority rule: uploaded file > precomputed demo scenarios
        if uploaded_txt is not None:
            ...
            missing = [p for p in DEMO_SCENARIO_CSVS.values() if not p.exists()]
            if missing:
                st.error(
                    "Demo scenario files not found:\n"
                    + "\n".join([f"- {p.as_posix()}" for p in missing])
                )
                st.stop()

            scenarios = load_demo_scenarios(DEMO_SCENARIO_CSVS)
            source_used = "Demo dataset (precomputed Jan 2026 scenarios)"
