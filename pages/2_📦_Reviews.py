...
# ─────────────────────────────────────────────────────────────────────────────
# DATA LOADING — demo dataset by default, optional upload to try your own
# ─────────────────────────────────────────────────────────────────────────────
DEMO_DIR = Path("data/demo")
DEMO_FILES = {
    "per_restaurant":    DEMO_DIR / "final_top_10_insights.csv",
    "common_insights":   DEMO_DIR / "common_insights_all_restaurants.csv",
    "category_insights": DEMO_DIR / "top_insights_by_category.csv",
    "global_insights":   DEMO_DIR / "global_insights_v2.csv",
}

st.sidebar.markdown("### Data source")
st.sidebar.caption("Showing demo insights by default. Upload your own CSVs below to try them instead.")
...
if up_per and up_com and up_cat:
    ...
    missing_demo = [p for p in DEMO_FILES.values() if not p.exists()]
    if missing_demo:
        st.error(
            "Demo data files not found:\n"
            + "\n".join([f"- {p.as_posix()}" for p in missing_demo])
        )
        st.stop()

    per_rest_df = load_csv(DEMO_FILES["per_restaurant"])
    common_df   = load_csv(DEMO_FILES["common_insights"])
    category_df = load_csv(DEMO_FILES["category_insights"])
    global_df   = load_csv(DEMO_FILES["global_insights"])
    st.sidebar.info("📊 Showing demo dataset")
...
""")
    ...
    with tab1:
        ...
        st.info("ℹ️ The category **FEATURES** is not taken into consideration in this summary.")
        c1, c2 = st.columns(2)
        with c1:
        ...
        cat_summary = (
            global_df_cat.groupby("category")
            .agg(
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    map_path = Path("images/selected_madrid_cafes_map_20260210.html")
    if map_path.exists():
